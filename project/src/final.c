...
void ledControl(int vlhkost, int teplota);
...
int main(void) {
    ...
    while (1) {
        int vlhkost = nactiVlhkost();    // Read soil moisture
        ...
        ledControl(vlhkost, dht12.temp_int);
...
/* Read soil moisture ------------------------------------------------*/
int nactiVlhkost(void) {
    int hodnota = analogRead(ANALOG_IN);
    if (hodnota > ADC_MIN) hodnota = ADC_MIN;
    if (hodnota < ADC_MAX) hodnota = ADC_MAX;
    return (ADC_MIN - hodnota) * 100 / (ADC_MIN - ADC_MAX);
}

/* Control LEDs based on conditions ----------------------------------*/
void ledControl(int vlhkost, int teplota) {
    if (vlhkost < set_moisture) PORTD |= (1 << WATERING_LED), PORTD &= ~(1 << DRYING_LED);
    else PORTD |= (1 << DRYING_LED), PORTD &= ~(1 << WATERING_LED);
    if (teplota < set_temp) PORTD |= (1 << HEATING_LED), PORTD &= ~(1 << COOLING_LED);
